...
Public Class ReportRepairFrm
        ...
        Try
            ...
            If slectsql = 100 Then
                strSQL = "SELECT sale_id,fix_id,customer_id,customer_name,total_all,datetime_save FROM cash_sale WHERE " _
                    ...
                    & "'" & Convert.ToDateTime(DateTimePicker2.Value).ToString("yyyy-MM-dd", CultureInfo.CreateSpecificCulture("en-EN")) & "' AND NOT(fix_id='" & "ขายสินค้า" & "')"
            ...
            ElseIf slectsql = 300 Then
                strSQL = "SELECT sale_id,fix_id,customer_id,customer_name,total_all,datetime_save FROM cash_sale WHERE " _
                    & "datetime_save='" & Now.ToString("yyyy-MM-dd", CultureInfo.CreateSpecificCulture("en-EN")) & "' AND NOT(fix_id='" & "ขายสินค้า" & "')"
    ...
    Private Sub btnRRviewProduct_Click(sender As Object, e As EventArgs) Handles btnRRviewProduct.Click
        Try
            ...
            Else
                Try
                    strSQL = "SELECT code_pro FROM cash_sale_barcode WHERE fix_id='" & dgReportRepair.SelectedCells.Item(2).Value & "'"
                    Mainfrm.lblMainStatus.Text = strSQL
                    cmd = New MySqlCommand(strSQL, ConnectionDB)
                    Call open_connection()
                    Dr = cmd.ExecuteReader()
