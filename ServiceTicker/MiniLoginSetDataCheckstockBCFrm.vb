...
Public Class MiniLoginSetDataCheckstockBCFrm
    ...
    Private Sub saveLogs()
        Try
            ...
            strSQL = "INSERT INTO applogs(user_id,user_name,user_usernamelogin,note,date_in)VALUES('" & uid & "','" & uname & "','" & Me.txtMLUusername.Text & "','" & "เข้าเตรียมข้อมูลเช็คสต็อกสินค้าบาร์โค๊ต" & "','" & DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.CreateSpecificCulture("en-EN")) & "')"
            cmd = New MySqlCommand(strSQL, ConnectionDB)
            Call open_connection()
            cmd.ExecuteNonQuery()
            uid = Nothing
    ...
    Private Sub ok()
        Try
            ...
            If sqlRead.Read Then
                Call ckUser()
                If Lev = "1" Or "2" Or "3" Then
                    Me.Cursor = Cursors.WaitCursor
                    Me.Hide()
                    Call CheckStoclBarcodeFrm.StartDataCheckStockBC()
                    Call saveLogs()
