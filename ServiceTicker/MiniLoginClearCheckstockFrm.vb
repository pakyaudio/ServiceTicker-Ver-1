...
Public Class MiniLoginClearCheckstockFrm
    ...
    Private Sub saveLogs()
        Try
            ...
            strSQL = "INSERT INTO applogs(user_id,user_name,user_usernamelogin,note,date_in)VALUES('" & uid & "','" & uname & "','" & Me.txtMLUusername.Text & "','" & "เข้าเคลียร์ข้อมูลเช็คสต็อก" & "','" & DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.CreateSpecificCulture("en-EN")) & "')"
            cmd = New MySqlCommand(strSQL, ConnectionDB)
            Call open_connection()
            cmd.ExecuteNonQuery()
            uid = Nothing
            ...
            If sqlRead.Read Then
                ...
                ElseIf Lev = "2" Or "3" Then
                    If MessageBox.Show("แน่ใจว่าต้องการเคลียร์ข้อมูลการตรวจสอบสต็อก", "ยืนยัน", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                        ...
                        Call open_connection()
                        cmd.ExecuteNonQuery()
                        CheckStockFrm.dgCSDontCheck.Rows.Clear()
                        strSQL = "TRUNCATE check_stock_cache"
                        cmd = New MySqlCommand(strSQL, ConnectionDB)
                        Call open_connection()
                        cmd.ExecuteNonQuery()
                        CheckStockFrm.dgCScheck.Rows.Clear()
                        Mainfrm.lblMainStatus.Text = "เคลียร์ข้อมูลการตรวจสอบสต็อกเรียบร้อยแล้ว"
                        CheckStockFrm.Button1.Enabled = False
                        Call saveLogs()
                        Me.Close()
                    Else
                        Me.txtMLUusername.Clear()
