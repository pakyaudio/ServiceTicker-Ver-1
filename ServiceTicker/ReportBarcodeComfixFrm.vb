...
Public Class ReportBarcodeComfixFrm
    ...
    Private Sub searchDatadgReport()
        Try
            ...
            ElseIf selectfun = 300 Then
                strSQL = "SELECT code_pro,bar_code,name_pro,total_pro,price_cost,price_buy,datetime_save FROM cash_sale_barcode WHERE datetime_save LIKE '%" & Now.ToString("yyyy-MM-dd", CultureInfo.CreateSpecificCulture("en-EN")) & "%' AND NOT (fix_id='" & "ขายสินค้า" & "')"
            ElseIf selectfun = 400 Then
                strSQL = "SELECT code_pro,bar_code,name_pro,total_pro,price_cost,price_buy,datetime_save FROM sale_barcode WHERE datetime_save LIKE '%" & Now.ToString("yyyy-MM-dd", CultureInfo.CreateSpecificCulture("en-EN")) & "%' AND NOT (fix_id='" & "ขายสินค้า" & "')"
            End If
        ...
        Try
            ...
            For sumn As Integer = 0 To Me.dgReportRepair.Rows.Count - 1
                ss2 += Me.dgReportRepair.Rows(sumn).Cells(4).Value
            Next
            'นับจำนวนแถว
            Me.txtmoney.Text = ""
    ...
    'Private Sub searchDatadgReportsalebarcode()
    ...
    '                .Rows(r).Cells(3).Value = Dr.Item("name_pro")
    '                .Rows(r).Cells(4).Value = Dr.Item("total_pro")
    '                .Rows(r).Cells(5).Value = Dr.Item("price_cost")
    ...
    '        For sum As Integer = 0 To Me.dgReportRepair.Rows.Count - 1
    '            Me.dgReportRepair.Rows(sum).Cells(0).Value = sum + 1
    '            Me.dgReportRepair.Rows(sum).Cells(8).Value = Me.dgReportRepair.Rows(sum).Cells(6).Value * Me.dgReportRepair.Rows(sum).Cells(4).Value
    '            Me.dgReportRepair.Rows(sum).Cells(7).Value = Me.dgReportRepair.Rows(sum).Cells(5).Value * Me.dgReportRepair.Rows(sum).Cells(4).Value
    '        Next
    '        'นับจำนวนเงิน
    '        If no = 0 Then
    '            Me.txtmoney.Text = "รวมราคาทุน 0 บาท " & " รวมราคาขาย 0 บาท"
    '        Else
    '            For sum As Integer = 0 To Me.dgReportRepair.Rows.Count - 1
    '                Dim ss, ss1 As Integer
    '                ss += Me.dgReportRepair.Rows(sum).Cells(7).Value
    '                ss1 += Me.dgReportRepair.Rows(sum).Cells(8).Value
    '                Me.txtmoney.Text = "รวมราคาทุน " & ss.ToString & " บาท " & " รวมราคาขาย " & ss1.ToString & " บาท"
    '            Next
    '        End If
    '        Dr.Close()
    '    Catch ex As Exception
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        ...
        Else
            ...
            ReportBarcodeComfixPrintFrm.Show()
        End If
    End Sub
End Class