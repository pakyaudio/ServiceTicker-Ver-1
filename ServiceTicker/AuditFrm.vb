...
Public Class AuditFrm
    ...
    Private Sub day()
        Try
            Dim today As Date
            Dim answer As Date
            today = Now.ToString("yyyy-MM-dd", CultureInfo.CreateSpecificCulture("en-EN"))
    ...
    Private Sub getDataaudit_outgoings()
        Try
            Me.dgAD.Rows.Clear()
            strSQL = "SELECT namewiden,list,money,note FROM audit_outgoings WHERE datetime='" & Now.ToString("yyyy-MM-dd", CultureInfo.CreateSpecificCulture("en-EN")) & "'"
            Mainfrm.lblMainStatus.Text = strSQL
            cmd = New MySqlCommand(strSQL, ConnectionDB)
            Call open_connection()
            Dr = cmd.ExecuteReader
            ...
            If MessageBox.Show("คุณแน่ใจว่าจะบันทึกข้อมูลนี้", "แจ้งเตือน", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                Try
                    ...
                    Using cmd = New MySqlCommand(strSQL, ConnectionDB)
                        cmd.Parameters.AddWithValue("@datetime", Now.ToString("yyyy-MM-dd", CultureInfo.CreateSpecificCulture("en-EN")))
    ...
    Private Sub DelDataaudit_outgoings()
        If MessageBox.Show("คุณแน่ใจว่าจะลบข้อมูลนี้", "แจ้งเตือน", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
            Try
                strSQL = "DELETE FROM audit_outgoings WHERE datetime='" & Now.ToString("yyyy-MM-dd", CultureInfo.CreateSpecificCulture("en-EN")) & "' AND namewiden='" & Me.dgAD.SelectedCells.Item(1).Value & "' AND list='" & Me.dgAD.SelectedCells.Item(2).Value & "' AND money='" & Me.dgAD.SelectedCells.Item(3).Value & "' AND note='" & Me.dgAD.SelectedCells.Item(4).Value & "' LIMIT 1;"
                Using cmd = New MySqlCommand(strSQL, ConnectionDB)
                    Call open_connection()
                    cmd.ExecuteNonQuery()
                End Using
            ...
            End Try
        Else
            Mainfrm.lblMainStatus.Text = "การลบข้อมูลถูกยกเลิก"
    ...
    Private Sub getDataAudit()
        Try
            dt = New DataTable
            strSQL = "SELECT money1,money2,money5,money10,money20,money50,money100,money500,money1000,totalAll,deliverMorning,deliverNoon,deliverEvening,totalDeliver FROM audit WHERE datetime='" & Now.ToString("yyyy-MM-dd", CultureInfo.CreateSpecificCulture("en-EN")) & "' LIMIT 1"
    ...
    Private Sub SaveAll()
        Try
            If MessageBox.Show("คุณแน่ใจว่าจะบันทึกข้อมูลนี้" & vbNewLine & "ข้อมูลนี้สามารถแก้ไขและบันทึกได้จนกว่าจะเลยเวลาเที่ยงคืน" & vbNewLine & "หากเลยเที่ยงคืนจะไม่สามารถแก้ไขได้อีก", "แจ้งเตือน", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                strSQL = "SELECT datetime FROM audit WHERE datetime='" & Now.ToString("yyyy-MM-dd", CultureInfo.CreateSpecificCulture("en-EN")) & "'"
                cmd = New MySqlCommand(strSQL, ConnectionDB)
                Call open_connection()
                Dr = cmd.ExecuteReader()
                Dr.Read()
                If Not Dr.HasRows Then
                    strSQL = "INSERT INTO audit(datetime,money1,money2,money5,money10,money20,money50,money100,money500,money1000,totalAll,`check`,transfer,cash,charges,deliverMorning,deliverNoon,deliverEvening,totalDeliver,difference)" _
                         & "VALUES(@datetime,@money1,@money2,@money5,@money10,@money20,@money50,@money100,@money500,@money1000,@totalAll,@check,@transfer,@cash,@charges,@deliverMorning,@deliverNoon,@deliverEvening,@totalDeliver,@difference)"
                    cmd = New MySqlCommand(strSQL, ConnectionDB)
                    cmd.Parameters.AddWithValue("@datetime", Now.ToString("yyyy-MM-dd", CultureInfo.CreateSpecificCulture("en-EN")))
                ...
                Else
                    strSQL = "UPDATE audit SET money1='" & Me.nudAD1.Text & "',money2='" & Me.nudAD2.Text & "'," _
                        ...
                        & "difference='0' WHERE datetime='" & Now.ToString("yyyy-MM-dd", CultureInfo.CreateSpecificCulture("en-EN")) & "'"
                    cmd = New MySqlCommand(strSQL, ConnectionDB)
                    Call open_connection()
                    cmd.ExecuteNonQuery()
                    Mainfrm.lblMainStatus.Text = "ปรับปรุงข้อมูลแล้ว"
                End If
            Else
                Mainfrm.lblMainStatus.Text = "การบันทึก Audit ถูกยกเลิก"
            End If
        Catch ex As Exception
    ...
    Private Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
        Me.lblADtime.Text = Format(Now, "yyyy-MM-dd hh:mm:ss")
